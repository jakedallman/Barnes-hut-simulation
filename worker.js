...
///Planet class to store values for each planet for calculation + position storage
class Planet {
    ...
    ///Uses Verlet integration to mostly conserve energy and update positions
    gravityUpdates(){
    ...
    ///So you can distunguish asteroids + planets
    isAsteroid(){
        return false;
    }
}
...
///Asteroid class to store values for each planet for calculation + position storage
class asteroid {
    constructor(name, radius,x,y,z, vx,vy,vz, color, mass){
        this.name = name;
        this.radius = radius*10;
...
///Populates the buffer array with proper position and radius data
...
/// Function for roughly approximating the orbital velocity
///required to have a circular orbit around the sun
...
///Creates all of the planets and asteroid objects
function initializeSimulation({ selectedMode, asteroids = 0, iceBodies = 0, randomMasses = false, massMin = 1, massMax = 100,
    ...
    sun = null;
    delatTime = 0.001;
    if (selectedMode === 'chaos') { ///Creates a giant square of particles with given inputs
    ...
    else if (randomMasses){ ///If random masses is selected, solar system is populated normally, but with random masses
        ...
        for (let i = 0; i < asteroids; i++) { ///Asteroid belt
        ...
        for (let i = 0; i < iceBodies; i++) {///Kuiper belt
    ...
    } else{///The above but with normal masses
...
///handles different messages from main thread
self.onmessage = function(event) {
    const data = event.data;
    if (data.type === 'start') { ///Starts the simulation and sends the first frame
        freeBuffers = data.buffers.slice();
        if (freeBuffers.length > 0) {
            sendFrame(freeBuffers.shift());
        }
    } else if (data.type === 'configure') {///Recieved configuration data + begins initialization
        initializeSimulation(data);
        if (freeBuffers.length > 0) {
            sendFrame(freeBuffers.shift());
        }
    } else if (data.type === 'returnBuffer') { ///Recieves the buffer back from main thread and sends the next frame
